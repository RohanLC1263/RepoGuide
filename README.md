...
**Not published.** No VS Code Marketplace listing yet -- install from a locally-built `.vsix` or via F5 debug launch only.
...
**RepositoryBrain is wired but empty.** RepoGuide has a long-term memory subsystem (`RepositoryBrain`) intended to accumulate knowledge across sessions -- git history, decisions, incidents -- and it's a real, registered provider in the live retrieval path today. But the ingestion pipelines that would populate it (git history, ADRs, code review, coverage) haven't been connected or built yet, so on a real test workspace it currently holds zero rows. That's an ingestion-source gap, not a query-side or architecture gap -- the retrieval path itself is finished and code-verified end-to-end. Retrieval quietly gets nothing from it; nothing is broken, there's just no compounding memory yet.

**Not exhaustively tested outside two real dogfood codebases.** The evaluation numbers above come from one real production repository plus a handful of held-out open-source corpora per language (see `docs/engineering-log/*_SEMANTIC_PROVIDER_REPORT.md` files). It has not been run against a wide variety of codebases or languages in combination.
...
| `get_dependencies` | The reverse of `get_dependents`: what a symbol or file itself calls/reads/imports/instantiates/falls back to, each with its file/symbol/line and relationship | Understanding what a symbol relies on before refactoring or removing something it uses -- "what would I break in the other direction." |
