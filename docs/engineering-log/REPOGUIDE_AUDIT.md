...
**Evidence:**
- 8-9 separately-invoked webview panels exist: chat sidebar (`src/ui/sidebarProvider.ts`), doc report (`docReportPanel.ts`), explain (`explainPanel.ts`), index health (`indexHealthPanel.ts`), investigation/plan tracker (`phase10Panels.ts`, 2 panels -- the Orientation panel that used to share this file was removed 2026-07-10, low-value once Key Modules and the never-implemented project synthesis were stripped out, leaving only a duplicate of Entry Points), memory explorer (`memoryExplorerPanel.ts`), daily brief (`dailyBriefPanel.ts`), notes (`notesPanel.ts`) — ~4,800 combined lines of UI code across 16 files, each its own mental model (evidence trail, plan completion %, memory search, coverage %, etc.), with no unifying dashboard.
